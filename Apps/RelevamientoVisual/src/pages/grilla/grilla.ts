...
export class GrillaPage {
  ...
  async SacarFoto() {
    ...
    let date = new Date();
    let imageName = `${date.getFullYear()}-${date.getMonth()+1}-${date.getDate()}-${date.getHours()}-${date.getMinutes()}-${date.getSeconds()}`;
    ...
    try {
      ...
      pictures.putString(image, "data_url").then(() => {
        ...
        pictures.getDownloadURL().then((url) => {
          ...
          baseRef.push({ "usuario": this.usuario.mail, "url": url, "votos": 0, "imageName": imageName });
        });
      });
    } catch (error) {
  ...
  async SubirFotos() {
    ...
    let date = new Date();
    let imageName = `${date.getFullYear()}-${date.getMonth()+1}-${date.getDate()}-${date.getHours()}-${date.getMinutes()}-${date.getSeconds()}-${date.getMilliseconds()}`;
